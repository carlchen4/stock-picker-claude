...
SECTOR_METRICS = {
    "Energy": [
        ("nd_ebitda", 0.25, False),   # net debt / EBITDA — leverage
        ("fcf_yield", 0.25, True),    # FCF / market cap
        ("roe",       0.20, True),
        ("ebitda_m",  0.15, True),
        ("div_yield", 0.10, True),
        ("rev_g",     0.05, True),
    ...
    "Utilities": [
        ("div_yield", 0.20, True),
        ("payout",    0.20, False),   # lower payout = safer dividend
        ("nd_ebitda", 0.20, False),
        ("roe",       0.15, True),
        ("eps_g",     0.15, True),    # rate-base growth proxy
        ("ebitda_m",  0.10, True),
    ],
    ...
    "Materials": [
        ...
        ("roe",       0.20, True),
        ("nd_ebitda", 0.15, False),   # miners should carry little debt
