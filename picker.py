...
SLEEVE_TICKERS = {
    ...
    "XFN.TO": ("FIN", "在 WS Automated Investing 按 FIN Fuzzy 4:2:1 持有 (TOP保险+CM各15%, MID4大行各7%, BOT3只各4%, 见 ~/quant/FIN_portfolio.md)"),
}
    ...
    for i, ticker in enumerate(picks, 1):
        ...
        if is_sleeve(ticker):
            ...
            if ticker == "XUT.TO":
                ...
                extra = "FIN Fuzzy 4:2:1 (TOP保险+CM各15%, MID4大行各7%, BOT3只各4%, 见 ~/quant/FIN_portfolio.md)"
...
def build_report_html(picks, weights, panel_latest, top_features, regime,
    ...
    for ticker in picks:
        ...
        if ticker == "XUT.TO":
            ...
            company = "FIN (WS) — Fuzzy 4:2:1 (15%/7%/4%)"
        rows_html += f"""
        <tr>
          <td>
