...
⚠️ 优化审计结论(2026-06-13,事件研究+回测做完,别再回来调旋钮):
   每个参数都验过,数据一致地说"别动"——策略对这些选择不敏感 = 它是稳健的:
   • 持有天数:事件研究偏 5、5年回测偏 10,两法矛盾=测不准 → 留 10,别调。
   • VIX 阈值:28/26/30 收益乱跳=过拟合,不是 edge → 留 28(标准压力位)。
   • 保守桶:KO/XLE/ZEO/FTS 都与科技低相关、能源在 2022 还反向对冲 → 不换。
   • 加 GLD:平时拖累、危机时已切去科技、5年回测验不出 → 不加(黄金属核心仓非此工具)。
   • 入场等 VIX 见顶再进:VIX 平均 1.7 天就见顶,等待反而错过反弹、最差更糟 → 立即进。
   • 模型预测反弹:18 年仅 33 次 VIX>28 事件,必过拟合 → 不可行。
   真正剩的优化全是非模型的:放 TFSA/RRSP 跑(换手免税)、当小卫星仓、机械执行去情绪。
   本质:温和、稳健、edge 不大的择时对冲;别假装它是 alpha 印钞机。事件研究见 event_study.py。
